...
def data_2013():
    ...
    for a in pd.read_csv('Data/PM2.5/aqm2013.csv', chunksize=24):
        ...
        for pm in data:
            if type(pm) is type(pm) is float or type(pm) is int:
                var = pm
                add = add + var
            elif type(pm) is str:
...
if __name__ == "__main__":
    ...
    c = data_2015()
    d = data_2016()
    plt.plot(range(0, 365), a, label='2014')
    plt.plot(range(0, 364), b, label='2015')
    plt.plot(range(0, 365), c, label='2016')
    plt.plot(range(0, 121), d, label='2017')
    plt.xlabel('Day')
    plt.ylabel('PM 2.5')
